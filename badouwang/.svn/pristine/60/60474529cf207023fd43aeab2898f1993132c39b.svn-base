...
class ExamAction extends CommonAction{
    ...
    //热门考题
    public function hotPapers(){
        $param=$this->getData(array('uid','cpage','pagesize'), 'post');
         if($param['cid']){
            $limit=$this->initLimit($param);
            $hsql="select paper.*,an.sid from bd_ex_paper paper right join (select count(sid)sum,sid from bd_ex_record group by sid) an on an.sid=paper.id order by an.sid desc limit $limit[0],$limit[1]";
            $papers=M('Ex_paper')->query($hsql);
            $this->ajaxr(true, '','',$papers);
        }else{
            $this->ajaxr(false, '', '参数cid不正确');
        }
