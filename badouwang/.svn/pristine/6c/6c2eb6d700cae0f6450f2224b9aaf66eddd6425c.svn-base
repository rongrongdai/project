...
class TeachAction extends CommonAction{
    //获取附近家教列表
    public function getAroundTeach(){
        $param=$this->getData(array('Lng','Lat','cpage','pagesize','lprice','hprice'), 'get');
        if($param['Lng'] && $param['Lat']){
             $limit=$this->initLimit($param);
             import('Com.MapService');
             if($param['lprice'] && $param['hprice'] && $param['lprice']<$param['hprice']){
                 $filter=array('price',$param['lprice'].','.$param['hprice']);
             }
             $this->ajaxr(1, '', '',MapService::getAround('teach',array('Lng'=>$param['Lng'],'Lat'=>$param['Lat'],'cpage'=>$limit[0],'pagesize'=>$limit[1]),$filter));
        }else{
