...
class BasicConfigAction extends CommonAction{
    ...
    public function notice(){
        ...
        $page=$this->initPage();
        import('Com.PageHelper');
        $sum=M('Notice')->where("type='$type'")->count();
        $pagesize=10;
        PageHelper::init($pagesize, 10, $page['page'], $sum,'');
        $limit=  PageHelper::getLimit();
        $sql="select notice.*,city.name from bd_notice notice left join bd_city city on city.id=notice.cid where type='$type' limit $limit[min],$pagesize";
        $data=M('Notice')->query($sql);
        $this->assignData(array('datas'=>$data,'pages'=>  PageHelper::getPageInfos(),'type'=>$type), 'assign');
        $this->display();
    }
    public function addNotice(){
        if($_POST){
            $data=$this->getData(array('ntext','type','link','city_id'),'post');
            $data['cid']=$data['city_id'];unset($data['city_id']);$data['atime']=time();
            $res=M('Notice')->add($data);
            $this->pagego('添加通知成功！','添加通知失败',$res,'notice');
        }else{
           $position = ' 网站设置 > 友情链接 > 添加链接';
           $this->display();
        }
