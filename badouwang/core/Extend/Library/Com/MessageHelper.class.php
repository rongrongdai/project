...
class MessageHelper {
    /**
          ...
          * @param $tempId 模板Id
    */
    private static function sendTemplateSMS($to,$datas,$tempId,$code){
            //主帐号
             $accountSid= 'aaf98f894ee35d30014ef80e74da18b9';
             //主帐号Token
             $accountToken= '4102f4c08cf0498583182c03e7a0573e';
             //应用Id
             $appId='8a48b5514ee36774014ef822698a19a5';
             //请求地址，格式如下，不需要写https://
             ...
             if($result->statusCode!=0) {
                 $content='验证码是'.$code;
                 M('Sms')->add(array('mobile'=>$to,'content'=>$content,'timestamp'=>time(),'status'=>0,'emess'=>$result->statusMsg));
                 return array('code'=>0,'message'=>$result->statusMsg);
             }else{
                 $content='验证码是'.$code;
                 M('Sms')->add(array('mobile'=>$to,'content'=>$content,'timestamp'=>time(),'code'=>$code));
                 return array('code'=>1,'code'=>$code);
             }
    }
    public static function sendMessage($mobilephone){
        $check=  substr(rand(1000, 999999),0,4);
        $last=M('Sms')->where("mobile='$mobilephone'")->order('timestamp desc')->field('timestamp')->find();
        if($last){
            if((time()-(int)$last['timestamp'])>120){
                
                $data=array($check,2);
                return self::sendTemplateSMS($mobilephone,$data,"1",$check);
               
            }else{
                return array('code'=>0,'message'=>'2分钟只能发送一次验证码');
            }
        }else{
            $data=array($check,2);
            return self::sendTemplateSMS($mobilephone,$data,"1",$check);
