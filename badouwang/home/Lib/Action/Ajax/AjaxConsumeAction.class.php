...
class AjaxConsumeAction extends CommonAction{
    ...
    public function getClog(){
        ...
        if($type['type']){
            ...
            $uid=session('uid');
            $asql="select depoit.money count,s_timestamp timestamp,fields.money current,depoit.info,depoit.status from bd_depoit depoit left join bd_consume consume on consume.id=depoit.id left join bd_memberfields fields on fields.uid=depoit.uid where depoit.uid='$uid'";
