...
class AjaxTeachAction extends CommonAction{
	...
	public function getHot(){
		...
		$sql = "SELECT a.uid,a.real_name,a.id_front,a.t_prof,a.info,a.address,b.name bname,c.name cname,k.price2,k.id FROM bd_authentication a JOIN bd_teacher_km k ON a.uid=k.uid JOIN bd_classfiy b ON k.fid=b.id JOIN bd_classfiy c ON k.gid=c.id WHERE a.type=5 AND k.fid=$id LIMIT 4";
        $data = M()->query($sql);
	...
	public function platTeacher(){
		...
		$condition = "";
		$condition .= $district?"a.district=$district":"a.city=$city_id";
		$condition .= $fid?" and k.fid=$fid":"";
		$condition .= $gid?" and k.gid=$gid":"";

		if($price==='up'){
			$condition .= " order by k.price2 asc";
		}else if($price==='down'){
			$condition .= " order by k.price2 desc";
		}

		$pageNow = $arr['pages']?(int)$arr['pages']:1;
		$pageSize = 10;
		$sum = M()->query("SELECT COUNT(distinct a.real_name) sum FROM bd_teacher_km k JOIN bd_authentication a ON k.uid=a.uid WHERE $condition");
		$sum = $sum[0]['sum'];
		if($sum>$pageSize){
			$limit = ($pageNow-1)*$pageSize;
			$condition .= " limit $limit,$pageSize";
		...
		$sql = "SELECT k.id,k.price2,a.id_front,a.real_name,a.info,a.t_age,a.address,b.name bname,c.name cname FROM bd_teacher_km k JOIN bd_authentication a ON k.uid=a.uid JOIN bd_classfiy b ON b.id=k.fid JOIN bd_classfiy c ON c.id=k.gid WHERE $condition GROUP BY a.real_name";
		$res = M()->query($sql);
