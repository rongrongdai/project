...
class AjaxUserAction extends CommonAction{
   ...
   public function vuser(){
    ...
    public function Calendar(){
        $arr = $this->getData(array('cy','cm','from'),'post');
        $cY = $arr['cy'];
        $cM = $arr['cm'];
        ...
        //查询当月课表
        $uid = session('uid');
        $from = $arr['from']=='teacher'?"tuid=$uid":"stid=$uid";
    ...
    public function schedule(){
        $arr = $this->getData(array('xdate','from'),'post');
        $date = $arr['xdate'];
        $uid = session('uid');
        $condition = $arr['from']=='teacher'?"o.tuid=$uid":"stid=$uid";
        $condition .= " and date='{$date}' and k_confirm=1";
    ...
    public function modCourse(){
        $arr = $this->getData(array('id','from'),'post');
        if(!empty($arr['from'])){
            $condition = "ks.stid=a.uid";
            $sql = "SELECT ks.*,o.course,a.rname real_name,a.user_pic id_front FROM bd_teacher_ks ks JOIN bd_jjbm_order o ON ks.kid=o.id JOIN bd_memberfields a ON $condition WHERE ks.id=".$arr['id'];
        }else{
            $condition = "ks.tuid=a.uid";
            $sql = "SELECT ks.*,o.course,a.real_name,a.id_front FROM bd_teacher_ks ks JOIN bd_jjbm_order o ON ks.kid=o.id JOIN bd_authentication a ON $condition WHERE ks.id=".$arr['id'];
        }
        $res = M()->query($sql);
    ...
    public function surePay(){
        $arr = $this->getData(array('id','type'),'post');
        $id = $arr['id'];
        if(!empty($arr['type'])){
            $type = 2;
            $uid = M('Teacher_ks')->where("id=".$id)->getField('stid');
        }else{
            $type = 1;
            $uid = session('uid'); 
        }
        ...
        //修改指定约课的k_status
        $res = M('Teacher_ks')->where("id=$id and stid=$uid and k_status=0")->setField('k_status',1);
        if($res){
            //判断是否完成订单所有课时
            $k_res = M('Teacher_ks')->where("id=$id and stid=$uid")->find();
            $kid = $k_res['kid'];
            $sum = M()->query("SELECT o.hour,o.money,SUM(ks.classhour) sum_hour FROM bd_teacher_ks ks LEFT JOIN bd_jjbm_order o ON ks.kid=o.id WHERE ks.kid=$kid AND ks.k_status=1");
            if($sum[0]['hour'] === $sum[0]['sum_hour']){
                M('Jjbm_order')->where("id=$kid")->setField('status',1);
            }
        
            //添加支付记录
            $pay = $sum[0]['money']/$sum[0]['hour']*$k_res['classhour'];
            $data = array('yid'=>$id,'stid'=>$k_res['stid'],'tuid'=>$k_res['tuid'],'classhour'=>$k_res['classhour'],'pay'=>$pay,'type'=>$type,'ctime'=>time());
            $res1 = M('Teacher_ks_pay')->data($data)->add();
            if($res1){
                //向平台老师账户支付课酬
                $res2 = M('Memberfields')->where("uid=".$k_res['tuid'])->setInc('money',$pay);
            }
        }
